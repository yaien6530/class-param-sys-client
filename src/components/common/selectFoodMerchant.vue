...
<template>
  <el-dialog title="选择美食商家" :visible.sync="visible" @before-close="handleClose" width="600px" top="3vh">
    <!-- 筛选 -->
    <el-form :inline="true">
    ...
    <!-- 列表 -->
    <el-table v-loading="getLoading" :data="tableList" ref="multipleTable" @selection-change="multipleChange" :header-cell-style="val => Fun.getRowClass(val.rowIndex)">
      ...
      <el-table-column v-if="!multiple" align="center" label="操作" width="100" fixed="right">
        <template slot-scope="{row}">
          <el-button @click="singleChange(row)" type="text">{{singleSelection.gourmet_merchant_id == row.gourmet_merchant_id ? '已' : ''}}选择</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 确认
    handleSubmit() {
      const data = this.multiple ? this.multipleSelection : this.singleSelection;

...
export default {
    ...
    methods: {
        openDetail() {
            ...
            this.fileList = [];
            this.$emit("getOpenData", this.opendata);
        },
        // 检查图片是否全部上传完成
        checkAllSuccess() {
        ...
        handlefinishSelect() {
            ...
            if (this.submit_img_list && this.submit_img_list.length === 1) {
                this.$emit("successCBK", this.submit_img_list[0]);
                this.dialogVisible = false;
            } else if (!this.submit_img_list.length) {

...
<template>
  <div class="account-config-container">
    ...
    <el-table v-loading="getLoading" :data="tableList" :header-cell-style="val => Fun.getRowClass(val.rowIndex)">
      ...
      <el-table-column label="民宿名" prop="housing_name" min-width="160" align="center" />
      ...
      <el-table-column align="center" label="操作" width="80" fixed="right">
        <template slot-scope="{row}">
          <el-button @click="updateStatus(row.housing_comment_id)" type="text">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.Api.getHousingCommentList(data).then(res => {
        if (res.data.ret === 0) {
    ...
    // 修改状态
    updateStatus(housing_comment_id) {
      this.$confirm(`是否删除评论`, "提示", {
        ...
        this.Api.deleteHousingComment(params).then(res => {
          if (res.data.ret === 0) {

...
<template>
  <div class="account-config-container">
    ...
    <el-table v-loading="getLoading" :data="tableList" :header-cell-style="val => Fun.getRowClass(val.rowIndex)">
      ...
      <el-table-column label="厨师名" prop="chef_name" min-width="160" align="center" />
      ...
      <el-table-column label="操作" width="160" align="right" fixed="right">
        <template slot-scope="{row}">
          <el-button v-if="row.status == 0" @click="updateStatus(row.chef_id, 1)" type="text">展示</el-button>
          <el-button v-else-if="row.status == 1" @click="updateStatus(row.chef_id, 0)" type="text">隐藏</el-button>
          <el-button @click="toEdit(row.chef_id)" type="text">编辑</el-button>
          <el-button @click="updateStatus(row.chef_id, -1)" type="text">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.Api.geChefList(data).then(res => {
        if (res.data.ret === 0) {
    ...
    updateStatus(chef_id, status) {
      ...
      this.$confirm(`是否${text}`, "提示", {
        ...
        this.Api.changChefStatus(params).then(res => {
          if (res.data.ret === 0) {
